    ...
    for (int i = 0; i < static_cast<int>(msg.ranges.size()); i++) {
      ...
      if (current_polar.r > msg.range_min) {
        ...
        if (first_measurement) {
          ...
          double x_diff = last_measurement.x - current_measurement.x;
          double y_diff = last_measurement.y - current_measurement.y;
          const auto dst = sqrt(x_diff * x_diff + y_diff * y_diff);
